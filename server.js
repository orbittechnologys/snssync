...
async function chapterSyncFromCollection() {
  try {
    ...
    console.log(
      "Reverse Data synchronized successfully for collection:",
      "Chapters"
    );
  } catch (error) {
...
app.get("/download-pdf", async (req, res) => {
  const url =
    "https://neodealsstorageaccount.blob.core.windows.net/neodealscontainer/sns/8554ff13-abc2-4558-ba56-00b1d5ef7f36iemh107.pdf";
  const filename = path.basename(url);
  const filePath = path.resolve(__dirname, "books", filename);
